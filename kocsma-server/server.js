...
app.get('/get-current-question', (req, res) => {
   (async () => {
      ...
      const resultAQ = await mydb.query("SELECT activeQuestion FROM activeQuiz WHERE Pin=" + mysql.escape(req.query.pin));
      console.log(resultAQ);
      if (resultAQ[0] == null) {
         const resultLB = await mydb.query("SELECT score FROM leaderboard WHERE boardID=" + mysql.escape(req.query.boardID));
         if (resultLB[0] == null) {
            res.json({
...
app.post('/cancel', (req, res) => {
   ...
   console.log("PIN", req.query.pin)
   mydb.query("DELETE FROM leaderboard WHERE boardID IN (SELECT boardID FROM activeQuiz WHERE Pin=" + mysql.escape(req.query.pin) + ")");
...
app.post('/submit-quiz', jsonParser, (req, res) => {
   (async () => {
      ...
      const correctAnswers = await mydb.query("SELECT CA FROM questions WHERE quizID IN (SELECT quizID FROM activeQuiz WHERE Pin=" + mysql.escape(req.body.pin) + ")");
      ...
      const resultBoardID = await mydb.query("SELECT boardID FROM activeQuiz WHERE Pin=" + mysql.escape(req.body.pin));
      console.log(resultBoardID)
      await mydb.query("UPDATE leaderboard SET score=" + mysql.escape(score) + " WHERE boardID=" + mysql.escape(resultBoardID[0].boardID));
      await mydb.query("DELETE FROM activeQuiz WHERE Pin=" + mysql.escape(req.body.pin));
      res.json({
         "score": score
      });
   })();
...
app.get('/quiz-questions', function (req, res) {
   (async () => {
      ...
      const resultMax = await mydb.query("SELECT MAX(boardID) AS maxID FROM leaderboard");
      console.log(resultMax);
      var boardID = 0;
      if (resultMax[0].maxID != null) {
         boardID = resultMax[0].maxID + 1;
      }
      await mydb.query("INSERT INTO leaderboard (boardID,teamName,quizID) VALUES ?", [[[boardID, req.query.teamName, req.query.quizID]]]);

      //Save team into activeQuiz
      await mydb.query("INSERT INTO activeQuiz (Pin, boardID,TeamName,quizID,activeQuestion) VALUES ?", [[[pin, boardID, req.query.teamName, req.query.quizID, -1]]]);
...
app.get('/join-game', (req, res) => {
   (async () => {
      ...
      //Teamname
      const selectRows = await mydb.query("SELECT boardID, TeamName, activeQuestion FROM activeQuiz WHERE Pin=" + mysql.escape(req.query.pin));
...
app.get('/get-leaderboard', (req, res) => {
   (async () => {
      ...
      const quizresult = await mydb.query("SELECT COUNT(*) AS count FROM quizList");
      ...
      for (let i = 1; i < quizCount + 1; i++) {
         ...
         const quizName = await mydb.query("SELECT name FROM quizList WHERE id="+ mysql.escape(i));
         const questionCount = await mydb.query("SELECT COUNT(*) AS count FROM questions WHERE quizID="+ mysql.escape(i));
         var quiz = {
   ...
   (async () => {
     ...
     /* const result32= mydb.query("DROP TABLE activeQuiz");
      ...
      if (result9[0].namesCount == 0) {
         const result10 = await mydb.query("CREATE TABLE activeQuiz (Pin INT PRIMARY KEY, boardID INT, TeamName VARCHAR(255), quizID INT, activeQuestion INT)")
      ...
      if (result11[0].namesCount == 0) {
         const result12 = await mydb.query("CREATE TABLE leaderboard (boardID INT PRIMARY KEY, TeamName VARCHAR(255), quizID INT, score INT)");
